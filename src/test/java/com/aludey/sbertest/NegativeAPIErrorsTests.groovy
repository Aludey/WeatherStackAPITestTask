...
class NegativeAPIErrorsTests extends BaseAPITest {
    ...
    def makeRequestWithoutAccessKey() { //101 missing_access_key
        URL url = new URL(baseURL + "current?access_key=")
        return requestWithOnlyURL(url)
    }

    def makeRequestWithInvalidAccessKey() { //101 invalid_access_key
        URL url = new URL(baseURL + "current?access_key=d6cc6c389390088")
        return requestWithOnlyURL(url)
    }

    def makeRequestWithInvalidAPIFunction() { //103 invalid_api_function
        return requestWithFunctionAndParameters("maketea","&query=with lemon")
    }

    def makeRequestWOQuery() { //601 missing_query
        return requestWithFunctionAndParameters("current","&query=")
    }

    def makeCurrentRequestWithLanguageParameter(String language) { //605 invalid_language 105 function_access_restricted
        return requestWithFunctionAndParameters("current","&query=New York&language=" + language)
    }
    def makeCurrentRequestWithUnitParameter(String unit) { //605 invalid_language 105 function_access_restricted
